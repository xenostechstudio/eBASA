    ...
    <div class="fixed bottom-6 right-6 z-40" data-branch-switcher>
        <div class="relative">
            <button type="button" data-branch-trigger
                class="flex items-center gap-2 rounded-full border border-slate-300 bg-white p-3 text-sm font-semibold text-slate-900 shadow-2xl shadow-slate-900/20 transition hover:bg-slate-50 dark:border-white/15 dark:bg-slate-900 dark:text-white">
            ...
            <div data-branch-panel class="absolute bottom-16 right-0 hidden w-72 rounded-3xl border border-slate-300 bg-white p-4 text-slate-900 shadow-2xl backdrop-blur dark:border-white/10 dark:bg-slate-900/95 dark:text-white">
                ...
                <ul class="mt-3 max-h-96 space-y-2 overflow-y-auto pr-1">
                    <li>
                        <form method="POST" action="{{ route('branch.switch') }}">
                            ...
                            <button type="submit"
                                class="flex w-full items-center justify-between rounded-2xl border border-slate-300 px-3 py-2 text-left transition hover:border-slate-400 {{ $activeBranchId === 0 ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'bg-slate-50 text-slate-900 dark:bg-white/5 dark:text-white' }}">
                                <div>
                    ...
                    @foreach ($branchShortcuts as $branch)
                        <li>
                            <form method="POST" action="{{ route('branch.switch') }}">
                                ...
                                <button type="submit"
                                    class="flex w-full items-center justify-between rounded-2xl border border-slate-300 px-3 py-2 text-left transition hover:border-slate-400 {{ $activeBranchId === $branch->id ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'bg-slate-50 text-slate-900 dark:bg-white/5 dark:text-white' }}">
                                    <div>
