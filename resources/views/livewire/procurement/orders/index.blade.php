<div class="space-y-6">
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <x-stat.card label="Total Orders" :value="number_format($stats['totalOrders'])" description="All time" tone="neutral">
            <x-slot:icon>
                @svg('heroicon-o-document-text', 'h-5 w-5 text-slate-500')
            </x-slot:icon>
        </x-stat.card>
        ...
        <x-stat.card label="Approved" :value="number_format($stats['approved'])" description="Ready to receive" tone="info">
            <x-slot:icon>
                @svg('heroicon-o-check', 'h-5 w-5 text-sky-500')
            </x-slot:icon>
        </x-stat.card>

        <x-stat.card label="Received" :value="number_format($stats['received'])" description="Completed" tone="success">
            <x-slot:icon>
    ...
    <div class="rounded-2xl border border-slate-300 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
        <div class="border-b border-slate-100 px-5 py-4 dark:border-white/10">
            <div class="flex flex-wrap items-center justify-end gap-3">
                ...
                <x-table.export-dropdown aria-label="Export orders" />
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                        @foreach ($orders as $order)
                            @php
                                $isSelected = in_array($order->id, $selectedItems);
                                $statusColors = [
                            ...
                            <tr class="cursor-pointer transition hover:bg-slate-50 dark:hover:bg-white/5 {{ $isSelected ? 'bg-slate-50 dark:bg-white/5' : '' }}">
                                ...
                                <td class="whitespace-nowrap px-5 py-4">
                                    <p class="font-medium text-slate-900 dark:text-white">{{ $order->reference }}</p>
                                </td>
                                ...
                                <td class="whitespace-nowrap px-5 py-4">
                                    <span class="inline-flex items-center rounded-lg px-2 py-1 text-xs font-medium {{ $statusColors[$order->status] ?? 'bg-slate-100 text-slate-600' }}">
        ...
        @else
            <div class="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p class="mt-4 text-sm font-medium text-slate-500 dark:text-white/60">No orders found</p>
