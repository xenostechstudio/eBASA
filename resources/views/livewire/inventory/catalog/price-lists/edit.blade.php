    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <nav class="flex items-center gap-2 text-sm text-slate-500 dark:text-white/50">
                ...
                <span>/</span>
                <span class="text-slate-900 dark:text-white">{{ $priceList->name }}</span>
            </nav>
            <h1 class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">Edit Price List</h1>
            @if ($activeBranch)
                <p class="mt-1 text-sm text-slate-500 dark:text-white/60">{{ $activeBranch->name }}</p>
            @endif
        </div>
        <div class="flex items-center gap-3">
            <button wire:click="delete" wire:confirm="Are you sure you want to delete this price list?" type="button"
