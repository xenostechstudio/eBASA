<div class="space-y-6">
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <x-stat.card label="Total Bundles" :value="number_format($stats['totalBundles'])" description="Product bundles" tone="neutral">
            <x-slot:icon>
                @svg('heroicon-o-squares-2x2', 'h-5 w-5 text-slate-500')
            </x-slot:icon>
        </x-stat.card>
        ...
        <x-stat.card label="Avg. Items" :value="number_format($stats['avgItems'])" description="Per bundle" tone="info">
            <x-slot:icon>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5">
        <div class="border-b border-slate-100 px-5 py-4 dark:border-white/10">
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <x-table.export-dropdown aria-label="Export bundles" />
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                        @foreach ($bundles as $bundle)
                            @php $isSelected = in_array($bundle->id, $selectedItems); @endphp
                            <tr class="cursor-pointer transition hover:bg-slate-50 dark:hover:bg-white/5 {{ $isSelected ? 'bg-slate-50 dark:bg-white/5' : '' }}">
                                ...
                                <td class="whitespace-nowrap px-5 py-4">
                                    <span class="inline-flex items-center rounded-lg px-2 py-1 text-xs font-medium {{ $bundle->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-slate-100 text-slate-600 dark:bg-white/10 dark:text-white/60' }}">
        ...
        @else
            <div class="flex flex-col items-center justify-center py-16 text-center">
                @svg('heroicon-o-squares-2x2', 'h-12 w-12 text-slate-300 dark:text-white/20')
                <p class="mt-4 text-sm font-medium text-slate-500 dark:text-white/60">No bundles found</p>
