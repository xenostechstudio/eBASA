<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <h1 class="mt-1 text-2xl font-bold text-slate-900 dark:text-white">New Stock Adjustment</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-white/60">Adjust stock quantities for corrections, damage, or initialization.</p>
        </div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6 lg:col-span-2">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-white/50">Select warehouse, type, and reason for this adjustment.</p>

                <div class="mt-6 grid gap-4 md:grid-cols-2">
                    <div>
                        <label class="block text-xs font-medium uppercase tracking-wide text-slate-600 dark:text-white/50">Warehouse</label>
                        <div class="mt-2">
                    ...
                    <div>
                        <label class="block text-xs font-medium uppercase tracking-wide text-slate-600 dark:text-white/50">Type</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium uppercase tracking-wide text-slate-600 dark:text-white/50">Adjustment date</label>
                        <input type="date" wire:model="adjustment_date"
                            class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-slate-400 focus:outline-none focus:ring-0 dark:border-white/10 dark:bg-slate-950/40 dark:text-white">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" wire:model="reason"
                            ...
                            placeholder="e.g. Stock count correction, damaged goods, initial stock" />
                        @error('reason')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea wire:model="notes" rows="3"
                            class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-slate-400 focus:outline-none focus:ring-0 dark:border-white/10 dark:bg-slate-950/40 dark:text-white dark:placeholder:text-white/30"
                            placeholder="Optional notes for audit trail and future reference..."></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Adjustment Items</h3>
                        <p class="mt-1 text-sm text-slate-500 dark:text-white/50">Choose products and quantities to adjust.</p>
                    </div>
                ...
                <div class="mt-6 space-y-4" x-data x-on:adjustment-item-added.window="$nextTick(() => { $refs.itemsEnd && $refs.itemsEnd.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); })">
                    @error('items')
                        ...
                        <div class="flex items-start gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                            <div class="flex-1 grid gap-4 md:grid-cols-3">
                                ...
                                <div class="flex items-end text-xs text-slate-500 dark:text-white/60">
                                    <p>
                                        Positive values will <span class="font-semibold">{{ $type === 'addition' ? 'increase' : 'decrease' }}</span> stock.
                                    </p>
                                </div>
                            </div>
                    ...
                    @empty
                        <div class="flex flex-col items-center justify-center py-12 text-center">
                            @svg('heroicon-o-adjustments-horizontal', 'h-10 w-10 text-slate-300 dark:text-white/20')
            ...
            <div class="sticky top-24 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
                ...
                <dl class="mt-6 space-y-4 text-sm">
                    ...
                    <div class="border-t border-slate-200 pt-4 text-xs text-slate-500 dark:border-white/10 dark:text-white/60">
                        Review the quantities and warehouse carefully before saving to keep stock accurate.
                    </div>
                </dl>
            </div>
        </div>
